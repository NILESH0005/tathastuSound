    ...
    >
      ...
      >
        ...
        <ul className="hidden md:flex items-center gap-10 text-white/80 font-medium">
          ...
          >
            <span className="cursor-pointer hover:text-white transition py-2">
              Inventory ▾
            </span>
            ...
            {openInventory && (
              ...
              >
                ...
                <DropdownItem label="Lighting" to="/inventory/light" />
                {/* For future implementation */}
                {/* <DropdownItem label="Video" to="/inventory/video" /> */}
                {/* <DropdownItem label="Backline" to="/inventory/backline" /> */}
              </div>
            )}
          </li>
...
function NavItem({ label, link }) {
  ...
  if (isHashLink) {
    return (
      <li>
        <a href={link} className="hover:text-white transition relative group">
          {label}
          <span className="absolute left-0 -bottom-1 w-0 h-[2px] bg-gradient-to-r from-purple-500 to-blue-500 group-hover:w-full transition-all duration-500" />
        </a>
      </li>
    );
  }
